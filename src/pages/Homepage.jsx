...
const Homepage = () => {
     ...
     useEffect(() => {
        ...
        const fetchHeaderData = async () => {
            try {
                ...
                setState(prevState => ({ ...prevState, headerData: data.results[randomMovieIndex] }));
            } catch (error) {
    ...
    const handleKeyPress = async (event) => {
        if (event.key === 'Enter') {
            try {
                const apiKey = import.meta.env.VITE_TMDB_TOKEN;
                const response = await axios.get(`https://api.themoviedb.org/3/search/movie?api_key=${apiKey}&query=${state.searchQuery}`);
                const data = response.data;
                setState(prevState => ({ ...prevState, searchResults: data.results }));
            } catch (error) {
                        ...
                        <main className="font-DmSans bg-cover bg-center" style={backgroundImg}>
                            <nav className="flex justify-between px-5 lg:px-10 items-center py-5">
                                <div className="flex items-center gap-1 lg:gap-5">
                                    ...
                                    <p className="text-gray-100 text-xl font-extrabold text-shadow-custom">MovieBox</p>
                                </div>
                                ...
                                <div className="items-center gap-5 hidden lg:flex">
                                    <p className="text-gray-100 text-md font-medium text-shadow-custom">Sign in</p>
                                    <img className="w-[30px]" src={menu} alt="menu-logo" />
                                </div>
                            </nav>

                            <section className="py-40 px-10 flex flex-col justify-center gap-5">
                                <p className="text-gray-100 drop-shadow-2xl text-3xl font-extrabold tracking-wider font-DmSans text-shadow-custom">{state.headerData.title}</p>
                                ...
                                <div className="flex items-center gap-5 text-shadow-custom">
                                    <div className="flex gap-2 text-gray-100 text-[12px] text-shadow-custom">
                                        ...
                                        <p>{convertToTwoDecimalPlaces(state.headerData.vote_average)}/100</p>
                                    </div>
                                ...
                                <p className="text-gray-100 w-80 font-extrabold text-sm">{state.headerData.overview}</p>
                        ...
                        <section className="py-10 px-10">
                            {state.searchResults.length > 0 ? (
                                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                                    {state.searchResults.map((movie, index) => (
