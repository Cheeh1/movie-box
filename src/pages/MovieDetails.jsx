...
const MovieDetails = () => {
  ...
  const { movies, writers, directors, stars, genres, trailer, error, loading } =
    movieData;
    ...
    const fetchData = async () => {
      try {
        ...
        const videosRes = await axios.get(
          `https://api.themoviedb.org/3/movie/${id}/videos?api_key=${apiKey}&language=en-US&official=true`
        );
  ...
  return (
    <>
      {loading ? (
        ...
        <main
          className="flex flex-col lg:flex-row gap-20 items-center px-10 py-9 bg-cover bg-center lg:h-[120vh]"
          style={backgroundImg}
        >
          <section className="flex flex-col gap-20">
            <Link
              to="/"
              className="flex items-center gap-2 text-gray-100 text-xl cursor-pointer font-bold"
            >
              <i className="fa-solid fa-house"></i>
              <p className="text-shadow-custom">Home</p>
            </Link>
            <img
              className="rounded-2xl shadow-sm shadow-gray-500 h-96 lg:h-full"
          ...
          <section className="flex flex-col gap-5 text-gray-100">
            ...
            <div className="flex flex-col">
              <h1 className="lg:text-[40px] text-[30px] font-bold text-gray-100 text-shadow-custom">
                {movies.title}
              </h1>
              <div className="flex flex-col lg:flex-row lg:gap-3 items-start text-shadow-custom">
                <div className="flex gap-3">
                  <p className="border border-[#b1adad] rounded-md px-2 font-bold">
                    PG-13
                  </p>
                  <p className="lg:text-3xl text-xl -mt-2 lg:-mt-4 font-extrabold">
                    .
                  </p>
                ...
                <div className="flex items-start gap-3">
                  <div className="flex gap-3">
                    {genres.map((genre, index) => (
                      <p key={index} className="text-gray-100 font-bold">
                        {genre.name}
                      </p>
                    ))}
                  </div>
